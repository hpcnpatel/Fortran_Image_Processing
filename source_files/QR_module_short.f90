...
!*************************************************************************************
      subroutine QR_rsolv(a,n,np,dd,b)
...
!*************************************************************************
      subroutine QR_QTxb_non_square(a,nn,mm,cc,dd,b)
...
!*************************************************************************************
      subroutine QR_rsolv_non_square(a,mm,dd,b)
...
!                           ....
      ...
      if(size(b) < mm) then
         write(*,*) 'QR_rsolv_non_square: error; size(b) < mm'
         write(*,'(a,i0)') 'size(b)=',size(b)
         write(*,'(a,i0)') 'mm     =',mm 
         stop 'QR_rsolv_non_square: error; size(b) < mm'
      endif
...
12    continue
      ...
      end subroutine QR_rsolv_non_square
!*************************************************************************************
...
! is input as the right-hand side vector, and is overwritten with the solution vector on output.
      ...
      real(kind=rk)     :: a(:,:),b(:),cc(:),dd(:)
      real(kind=rk)     :: result(:)
      real(kind=rk)     :: temp(1000)                   
      integer           ::  i,j
      real(kind=rk)     :: sum,tau

!  form Q^T b
      temp(1:n)=b
      call QR_QTxb_non_square(a,n,mm,cc,dd,temp)

! solve R^(-1) b
      call QR_rsolv_non_square(a,mm,dd,temp)
      result(1:mm)=temp(1:mm)


      end subroutine QR_solv_non_square
!*************************************************************************************
      subroutine QR_QTxb_with_selection(a,nn,mm,QR,b)
...
!*************************************************************************************
      subroutine QR_Qxb_with_selection(a,nn,mm,QR,b)
